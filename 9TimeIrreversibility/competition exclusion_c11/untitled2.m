...
dim=3;
% dt=0.001;
D=40;%D=20;%小噪声下计算效果也不错

%区域
xmin=[0,0,0];
xmax=[150,150,150];
% randn (1,dim);
a1=0.01:0.0025:0.057;
b=length(a1)
for g=1:b
    g
    ...
    Cji=[a1(1,g),0.04,0.07;0.10,0.08,0.08;0.10,0.14,0.10];%Fig.2C
...
xlabel('C_{11}','FontSize',27)
ylabel('\tau','FontSize',27);
% plot([0.057 0.057],[3.15880482039652,7.554604820396519],'r--','LineWidth',1)
% axis([0.000145992945713,0.066030492836629,4.046734592930996,6.682114174048626])
hold on
...
set(gca,'LineWidth',1.2,'Fontsize',27)
set(gca,'TickDir', 'in', 'TickLength', [0.009 0.01])
% plot([0.057 0.057],[2.696056983135674,4.293256983135675],'r--','LineWidth',1)
% axis([0.003605804765925,0.063500804765925,2.696056983135674,4.293256983135675])
xlabel('C_{11}','FontSize',27)
ylabel('\tau','FontSize',27);
...
set(gca,'LineWidth',1.2,'Fontsize',27)
set(gca,'TickDir', 'in', 'TickLength', [0.009 0.01])
% plot([0.057 0.057],[2.012268507839097,5.555390507839098],'r--','LineWidth',1)
% axis([-0.007711422379328,0.072008822620672,2.012268507839097,5.555390507839098])
xlabel('C_{11}','FontSize',27)
ylabel('\tau','FontSize',27);
...
% set(gca,'LineWidth',1.2,'Fontsize',27)
% set(gca,'TickDir', 'in', 'TickLength', [0.009 0.01])
% plot([0.057 0.057],[53.961891533015226,113.8568915330152],'r--','LineWidth',1)
% axis([0.001997559638099,0.061892559638099,53.961891533015226,113.8568915330152])
ax = gca();
% ax.XRuler.Exponent = -2;
% ax.YRuler.Exponent = -2;
xlabel('C_{11}','FontSize',27)
...
% set(gca,'LineWidth',1.2,'Fontsize',27)
% set(gca,'TickDir', 'in', 'TickLength', [0.009 0.01])
% plot([0.057 0.057],[76.04404894073998,115.9740489407399],'r--','LineWidth',1)
% axis([0.004466420577639,0.064361420577639,76.04404894073998,115.9740489407399])
ax = gca();
% ax.XRuler.Exponent = -2;
% ax.YRuler.Exponent = -2;
xlabel('C_{11}','FontSize',27)
...
%保存数据
filename1 = ['data_c11=', num2str(0.01, '%.3f'), '--c11=', num2str(0.057, '%.3f'), '.mat'];
save(filename1);
